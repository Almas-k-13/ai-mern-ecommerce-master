...
const AiAssistant = () => {
  ...
  const askAI = async (message) => {
    try {
      const res = await axios.post("/api/ai/recommend", { message });
  ...
  return (
    ...
    >
      <div
        className={`w-16 h-16 rounded-full bg-emerald-500 flex items-center justify-center text-3xl transition-all duration-300 ${
          activeAi ? "scale-125 shadow-[0_0_40px_#10b981]" : ""
        }`}
      >
        🤖
      </div>
